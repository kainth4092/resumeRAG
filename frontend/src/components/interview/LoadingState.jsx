...
function Skel({ className = "" }) {
    return <div className={`bg-muted/70 animate-pulse rounded-xl ${className}`} />;
}
                ...
                {steps.map((s, i) => (
                    <div
                        ...
                        className={`flex items-center gap-3 p-3 rounded-xl transition-all ${
                            i <= step ? "bg-primary/5 border border-primary/12" : "opacity-25"
                        }`}
                    >
                        <div
                            className={`w-4 h-4 rounded-full flex items-center justify-center flex-shrink-0 ${
                                i < step ? "bg-emerald-500" : i === step ? "bg-primary animate-pulse" : "bg-muted"
                            }`}
                        >

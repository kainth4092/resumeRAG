...
export default function Toolbar({ search, setSearch, setBookmarkOnly, bookmarkOnly, diffFilter, setDiffFilter }) {
    return (
        <div className="flex flex-wrap items-center gap-2 pb-0.5">
            ...
            <div className="w-44 shrink-0"><Select options={DIFF_OPTS} value={diffFilter} onChange={setDiffFilter} size="sm" /></div>
            <button
                onClick={() => setBookmarkOnly((b) => !b)}
                className={`flex items-center gap-1.5 h-9 px-3 rounded-xl border text-sm font-semibold transition-all shrink-0 ${bookmarkOnly ? "bg-amber-500/10 border-amber-500/30 text-amber-600" : "border-border text-muted-foreground hover:bg-muted"
                    }`}
            >

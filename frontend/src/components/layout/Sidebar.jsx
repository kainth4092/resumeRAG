...
export default function Sidebar({ collapsed, setCollapsed, setMobileOpen }) {
    ...
    const user = { name: "Jordan Davis", email: "jordan@example.com", avatar: "JD" };
                ...
                {!collapsed ? (
                    <div className="flex items-center gap-2.5 min-w-0">
                        ...
                        <div className="min-w-0">
                            <p className="text-sm font-semibold text-sidebar-foreground leading-none truncate">ResumeRAG AI</p>
                            {/* <p className="text-[10px] text-primary font-semibold mt-0.5 tracking-wide">Pro Plan</p> */}
                        </div>
                    </div>
                ) : (
                            ...
                            {section.items.map(item => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`
                                            w-full flex items-center gap-3 rounded-xl text-sm transition-all duration-150 group relative
                                    ...
                                    >
                                        ...
                                        <Icon size={16} className={`flex-shrink-0 transition-colors ${active ? "text-primary" : ""}`} />

                                        {!collapsed && (
                                            <>
                                                <span className="flex-1 text-left truncate">{item.label}</span>
                                                {item.badge && (
                                                    <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full flex-shrink-0 ${item.badge === "New"
            ...
            <div className="flex-shrink-0 p-3 border-t border-sidebar-border">
                {collapsed ? (
                    <div className="flex justify-center">
                        <div className="w-8 h-8 rounded-xl bg-primary/15 flex items-center justify-center text-primary text-xs font-bold cursor-pointer">
                            {user.avatar}
                        </div>
                    </div>
                ) : (
                    <div className="flex items-center gap-3 p-2.5 rounded-xl hover:bg-sidebar-accent transition-all cursor-pointer group">
                        ...
                        <div className="flex-1 min-w-0">
                            <p className="text-sm font-semibold text-sidebar-foreground truncate leading-none">{user.name}</p>

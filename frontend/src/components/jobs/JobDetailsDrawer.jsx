...
export default function JobDetailsDrawer({
  ...
  return (
    <>
      ...
      <div className="fixed right-0 top-0 bottom-0 w-full max-w-[500px] bg-card border-l border-border z-50 flex flex-col shadow-[var(--shadow-lg)] animate-in slide-in-from-right duration-300">
          ...
          <div className="flex items-center gap-3 min-w-0">
            {logoUrl ? (
              <img
                src={logoUrl}
                alt={companyName}
                className="w-10 h-10 rounded-2xl flex-shrink-0 object-contain bg-muted border border-border"
                onError={(e) => {
        ...
        <div className="flex-shrink-0 p-5 border-t border-border bg-muted/20 flex gap-3">
          <button
            onClick={() => onSave(job)}
            disabled={saving}

function DeleteDialog({ resume, onConfirm, onCancel }) {
      ...
      <div className="relative w-full max-w-sm bg-card border border-border rounded-2xl shadow-[var(--shadow-lg)] p-6 animate-in fade-in-0 zoom-in-95 duration-200 space-y-4">
        ...
        <div className="flex gap-3 pt-1">
          <button
            onClick={onCancel}
            disabled={deleting}
            className="flex-1 py-2.5 rounded-xl border border-border text-sm font-semibold text-foreground hover:bg-muted active:scale-[0.98] disabled:opacity-50 transition-all"
          >
            Cancel
          </button>
          <button
            ...
            disabled={deleting}
            className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl bg-destructive text-white text-sm font-semibold hover:bg-destructive/90 active:scale-[0.98] disabled:opacity-70 transition-all"
          >
            {deleting ? (
              <>

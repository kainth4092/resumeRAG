...
export function Select({ options = [], value, onChange, placeholder = "Select...", size = "md" }) {
  ...
  return (
    <div className="relative w-full" ref={containerRef}>
      ...
      {isOpen && (
        <div className="absolute left-0 right-0 mt-1.5 bg-popover border border-border rounded-xl shadow-[var(--shadow-lg)] z-50 py-1 max-h-48 overflow-y-auto animate-in fade-in-0 zoom-in-95 duration-100">
          {options.map((opt) => (
            <button
              key={opt.value}
              type="button"
              ...
              className={`w-full px-3.5 py-2 text-left text-sm transition-colors cursor-pointer ${
                opt.value === value

export default function Modal({ title, children, onSave, onClose, saving }) {
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-lg bg-card border border-border rounded-2xl shadow-[var(--shadow-lg)] animate-in fade-in-0 zoom-in-95 duration-200 max-h-[90vh] flex flex-col">
                ...
                <div className="flex gap-3 px-5 py-4 border-t border-border flex-shrink-0">
                    <button onClick={onClose} className="flex-1 py-2.5 rounded-xl border border-border text-sm text-foreground hover:bg-muted transition-all">Cancel</button>
                    <button
                        onClick={onSave}
                        disabled={saving}

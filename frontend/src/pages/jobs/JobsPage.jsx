...
import StatusBadge, { STATUS_CFG } from "../../components/jobs/StatusBadge";
import Select from "../../components/resume/dashboard/Select";

import { useJobTracker } from "../../hooks/useJobTracker";
import ApplicationDrawer from "../../components/jobs/ApplicationDrawer";
...
export default function JobsPage() {
  ...
  const filteredJobs = jobs.filter((job) => {
    if (location) {
      ...
      if (location === "Remote Only" && !jobLoc.includes("remote")) return false;
      if (location !== "Remote Only" && !jobLoc.includes(location.toLowerCase())) return false;
    }
    if (jobType) {
      const jobTypeStr = (job.employment_type || job.type || "").toLowerCase();
      if (!jobTypeStr.includes(jobType.toLowerCase())) return false;
    }
    if (remote) {
      const jobLoc = (job.location || "").toLowerCase();
      const isRemote = jobLoc.includes("remote");
      if (remote === "yes" && !isRemote) return false;
      if (remote === "onsite" && isRemote) return false;
    }
    return true;
  });
    ...
    <div className="h-full overflow-y-auto bg-background">
      <div className="max-w-7xl mx-auto px-4 md:px-6 py-6 space-y-6">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            <h1 className="text-foreground text-3xl font-bold tracking-tight">Job Tracker</h1>
            <p className="text-sm text-muted-foreground mt-1 max-w-xl">
              Discover jobs, generate tailored resumes, and manage all your applications in one place.
            </p>
          </div>
          <div className="flex items-center gap-2 flex-shrink-0">
            <button
              onClick={handleExportCSV}
              className="flex items-center gap-2 h-9 px-4 rounded-xl border border-border text-sm font-semibold text-foreground hover:bg-muted hover:border-primary/30 active:scale-[0.97] transition-all cursor-pointer"
            >
              <Download size={14} className="text-muted-foreground" /> Export CSV
            </button>
            <button
              ...
              return (
                ...
                >
                  ...
                  {t.id === "applications" && trackedJobs.length > 0 && (
                    <span
                      className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${active ? "bg-primary text-white" : "bg-muted text-muted-foreground"
                        }`}
                    >
                      {trackedJobs.length}
                    </span>
            ...
            <div className="flex gap-4 overflow-hidden">
                      ...
                      <table className="w-full text-sm">
                        ...
                        <tbody>
                          {filteredApplications.map((app) => {
                            const isSelected = selectedApp?.job_id === app.job_id;
                            return (
                              <tr
                                key={app.job_id}
                                onClick={() =>
                                ...
                                className={`border-b border-border last:border-0 cursor-pointer transition-all group ${isSelected ? "bg-primary/5" : "hover:bg-muted/30"
                                  }`}
                              >
                                <td className="px-5 py-3.5">
                                  <div className="flex items-center gap-3">
                                    <div className="w-8 h-8 rounded-xl flex items-center justify-center text-xs font-bold bg-primary/10 text-primary flex-shrink-0">
                                ...
                                <td className="px-5 py-3.5">
                                  <div
                                    ...
                                    ref={
                                      menuOpen === app.job_id ? menuRef : undefined
                                    }
                                  >
                                    <button
                                      onClick={(e) => {
                                    ...
                                    <div className="relative">
                                      <button
                                        onClick={(e) => handleMenuClick(e, app.job_id)}
                                        className="w-7 h-7 flex items-center justify-center rounded-lg hover:bg-muted text-muted-foreground hover:text-foreground transition-all cursor-pointer"
                                      >
              ...
              {selectedApp && (
                <div className="hidden lg:block w-[420px] flex-shrink-0">
                  <div className="sticky top-0 bg-card border border-border rounded-2xl overflow-hidden shadow-[var(--shadow-md)] max-h-[calc(100vh-12rem)] flex flex-col">
                    <ApplicationDrawer
                      app={selectedApp}
      ...
      <JobDetailsDrawer
        ...
        isSaved={selectedJob ? savedJobIds.has(selectedJob.job_id || selectedJob.id) : false}
        onSave={handleSaveJob}
        saving={saving}
      />
      {addJobOpen && (
      ...
      {menuOpen && (() => {
        ...
        return (
          <div
            ref={menuRef}
            className="fixed w-52 bg-popover border border-border rounded-2xl shadow-[var(--shadow-lg)] z-[300] overflow-hidden animate-in fade-in-0 zoom-in-95 duration-150"

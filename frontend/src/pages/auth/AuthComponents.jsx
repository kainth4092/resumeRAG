...
export const AuthInput = forwardRef(
    ({ label, error, success, hint, icon, suffix, className = "", ...props }, ref) => {
...
export function PasswordInput({
    label, error, success, hint, value, onChange, placeholder, name, autoComplete,
}) {
...
function getStrength(pw) {
    ...
    const color = score <= 1 ? "bg-red-500" : score === 2 ? "bg-orange-500" : score === 3 ? "bg-amber-500" : score === 4 ? "bg-lime-500" : "bg-emerald-500";
...
export function PasswordStrengthMeter({ password }) {
    ...
    return (
        <div className="mt-2 space-y-2">
            ...
            <p className={`text-xs font-medium ${score <= 2 ? "text-red-500" : score === 3 ? "text-amber-500" : "text-emerald-600 dark:text-emerald-400"}`}>{label}</p>
...
export function AuthCheckbox({
    ...
    return (
        ...
        `}>
            ...
            {error && <p className="mt-1 text-xs text-destructive flex items-center gap-1"><AlertCircle size={11} />{error}</p>}
        </div>
    );
}

export function AuthButton({
    children, loading, disabled, onClick, type = "button", variant = "primary", className = "",
}) {
    const base = "w-full flex items-center justify-center gap-2.5 px-4 py-3 rounded-xl text-sm font-semibold transition-all duration-200 select-none focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-card";
    const variants = {
        primary: "bg-primary text-primary-foreground hover:bg-primary/90 active:scale-[0.98] focus:ring-primary disabled:opacity-50 disabled:cursor-not-allowed shadow-sm shadow-primary/20 hover:shadow-md hover:shadow-primary/25",
        secondary: "bg-muted text-foreground hover:bg-muted/70 active:scale-[0.98] focus:ring-primary/50 disabled:opacity-50",
        outline: "border border-border bg-card text-foreground hover:bg-muted hover:border-primary/40 active:scale-[0.98] focus:ring-primary/50 disabled:opacity-50",
    };
    return (
        <button
            type={type}

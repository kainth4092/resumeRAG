...
function JobCard({ job, isSaved, onSave, onViewDetails, saving }) {
  ...
  return (
    <div
      ...
      className="bg-card border border-border rounded-2xl p-5 hover:shadow-[var(--shadow-md)] hover:-translate-y-1 transition-all duration-200 cursor-pointer relative overflow-hidden group flex flex-col justify-between min-h-[190px]"
    >
      <div>
        <div className="flex items-start gap-3 mb-4">
          {logoUrl ? (
            <img
              src={logoUrl}
              alt={companyName}
              className="w-11 h-11 rounded-2xl flex-shrink-0 object-contain bg-muted border border-border"
              onError={(e) => {
          ...
          <div
            className="w-11 h-11 rounded-2xl flex items-center justify-center text-sm font-bold text-white flex-shrink-0 shadow-sm"
            style={{
      ...
      <div className="flex items-center gap-2 pt-3 border-t border-border mt-auto">
        <button
          ...
          className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl border border-border text-xs font-semibold text-foreground hover:bg-muted active:scale-[0.98] transition-all cursor-pointer"
        >
          <Eye size={12} />
          View Details
        </button>

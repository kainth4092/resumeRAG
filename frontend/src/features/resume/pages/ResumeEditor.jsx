...
export default function ResumeEditor() {
  ...
  const templateOptions = ["Professional", "ATS", "Minimal", "Corporate"].map(
    (name) => ({
      value: name,
      label: `${name} Template`,
    }),
      ...
      <div className="shrink-0 flex items-center gap-3 px-5 py-3.5 border-b border-border bg-card/80 backdrop-blur-xl relative z-20">
        ...
        <div className="flex items-center gap-2 ml-auto">
          ...
          <button
            onClick={handleSave}
            disabled={saving}
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold transition-all active:scale-[0.97] disabled:opacity-70 cursor-pointer ${
              saved

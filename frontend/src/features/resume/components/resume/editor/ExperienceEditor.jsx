    ...
    >
      <div className="space-y-5 text-left">
        {experience.map((exp) => (
          <div
            key={exp.id}
            className="p-4 border border-border rounded-xl bg-background/50 space-y-3 relative group"
          >
            <button
              onClick={() =>
                onChange(experience.filter((e) => e.id !== exp.id))
              }
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <Select
                    ...
                    onChange={(v) => updateExp(exp.id, "startMonth", v)}
                    size="sm"
                  />
                  <Select
                    options={YEARS(2000)}
                    value={exp.startYear}
                    onChange={(v) => updateExp(exp.id, "startYear", v)}
                    size="sm"
                  />
                </div>
              </div>
                ...
                {!exp.current && (
                  <div className="flex gap-2">
                    <Select
                      options={MONTHS}
                      value={exp.endMonth}
                      onChange={(v) => updateExp(exp.id, "endMonth", v)}
                      size="sm"
                    />
                    <Select
                      options={YEARS(2000)}
                      value={exp.endYear}
                      onChange={(v) => updateExp(exp.id, "endYear", v)}
                      size="sm"
                    />
                  </div>
                )}
                {exp.current && (
                  <div className="h-9 flex items-center px-3.5 bg-muted/50 border border-border rounded-xl text-xs text-muted-foreground">
                    Present
                  </div>
                )}
              </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                ...
                <button
                  ...
                  className="text-[11px] text-primary hover:text-primary/80 font-semibold flex items-center gap-1 transition-colors cursor-pointer"
                >
                  <Plus size={10} /> Add
                </button>
              </div>
              <div className="space-y-2">
                {exp.bullets.map((b, bi) => (
                  <div key={bi} className="flex items-start gap-2">
                    <span
                      className="mt-2.5 text-muted-foreground/50 shrink-0"
                    ...
                    {exp.bullets.length > 1 && (
                      <button
                        ...
                        className="mt-2.5 text-muted-foreground hover:text-destructive transition-colors shrink-0 cursor-pointer"
                      >
                        <X size={13} />
                      </button>
                    )}
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-2 py-3 rounded-xl border-2 border-dashed border-border text-sm text-muted-foreground hover:border-primary/40 hover:text-primary hover:bg-primary/3 transition-all cursor-pointer"
        >

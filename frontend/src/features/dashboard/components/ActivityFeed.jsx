          ...
          feed.map((act, i) => {
            ...
            return (
              <div key={i} className="flex gap-3 items-start group">
                <div className={`w-8 h-8 rounded-xl ${act.color || "bg-primary/10 text-primary"} flex items-center justify-center shrink-0`}>
                  <Icon size={14} />
                </div>
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-[10px] text-muted-foreground mt-0.5 truncate">{act.body}</p>
                </div>
              </div>
            );
          })

...
def upgrade() -> None:
    ...
    op.add_column(
        "users",
        sa.Column(
            "provider", sa.String(length=50), nullable=True, server_default="local"
        ),
    )
    op.add_column(
    ...
    op.alter_column(
        "users", "password_hash", existing_type=sa.VARCHAR(length=100), nullable=True
    )
...
def downgrade() -> None:
    """Downgrade schema."""
    op.alter_column(
        "users", "password_hash", existing_type=sa.VARCHAR(length=100), nullable=False
    )
